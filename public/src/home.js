...
function getFiveHighestCounts(unsortedArray) {
  ...
  const sortedArray = unsortedArray.sort((optionA, optionB) => optionB.count - optionA.count);
  const fiveHighest = []
...
function getKeyCount(object, key) {
  ...
  const keyCount = keyArray.reduce((result, keyAppearance) => {
    result[keyAppearance] ? result[keyAppearance] += 1 : result[keyAppearance] = 1;
    return result;
  }, {})
  return keyCount;
}
...
function getMostPopularAuthors(books, authors) {
  ...
  for (const authorId in authorBorrows) {
    ...
    authorIdBorrowsArray.push({id: Number(authorId), count: authorBorrows[authorId]})
  }
  // Use helper function to get just the five highest counts
  ...
  return sortedAuthorIds.map(author => {
    ...
    return {name: `${authorName.first} ${authorName.last}`, count: author.count}
  })
}
